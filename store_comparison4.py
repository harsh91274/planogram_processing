...
files2=files.sort_values(by=['Datetime','Store','Location'],ascending=True);    #sort files
...
for i in range (file_count):
    ...
    out_fields1=[sl+" "+s+" "+my for s in fields1]; #create labels 1 for output columns
    out_fields2=[sl+" "+s+" "+my for s in fields2]; #create labels 2 for output columns
    out_fields3=[sl+" "+s+" "+my for s in fields3]; #create labels 2 for output columns
    out_fields=fields_m+out_fields2+out_fields1+out_fields3;    
    df_dw1=pd.DataFrame(columns=out_fields);    #creating iteration output dataframe 
    df_dw1[fields_m]=df[fields_m];  #copying model#   
    #df_dw1[fields_m1]=df[fields_m1]; 
    ...
    if df.empty==False:
        ...
        df_dw=pd.merge(left=df_dw, right=df_dw1, how='outer',left_on=['Model #', 'Brand / Appliance', 'MSRP'], right_on=[str(fields_m[0]),str(fields_m[1]),str(fields_m[2])]); #outer merge of datasets             

if df_dw.empty==False:    
    drop_cols = [col for col in df_dw.columns if 'Planogram' in col];   #drop empty rows
    df_dw_master=df_dw.dropna(subset=drop_cols, how='all').reset_index(drop=True);  #MASTER DATAFRAME
    df_dw_master["Brand / Appliance"]=df_dw_master["Brand / Appliance"].str.strip();
    df_dw_master2=distances(df_dw_master, file_count, files2, fields1, fields2, fields3);
    correlation(df_dw_master2,file_count,files2,fields_corr,path_plot);
    df_dw_master3=qmodel(df_dw_master2, file_count, files2, fields_q, path_plot, fields_pred);
